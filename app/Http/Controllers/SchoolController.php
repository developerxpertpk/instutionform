...
class SchoolController extends Controller
{
    ...
    {
        $test = $this->test_validation($request->all());

        if($test != 'true'){
            return back()->withInput()->withErrors($test);
        }
            ...
            // code to upload th pdf file or documents
            if($request->hasfile('document')){
                $documents = $request->file('document');
    ...
    public function test_validation($data)
    {
        $rules = array(
            'school_name' => 'required',
            'school_address' => 'required',
